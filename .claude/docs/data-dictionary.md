...
| `CustomStore`            | Tienda personal abierta: `Name` (PK), `Active` (tinyint), `Type` (smallint), `StoreName` (varchar 36). **No tiene datos de ítems** — el ítem real vive en `Character.Inventory` (binario), esta tabla solo dice qué personaje tiene tienda abierta y su nombre. `Active=1` confirmado contra datos reales (2026-08-11). |
