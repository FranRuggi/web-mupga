...
- 2026-09-04 — [Fix] Registro: no se podía crear una segunda cuenta, justo cuando la mayoría quiere 3 (main + mulas). Eran dos cosas distintas: (1) `register.php` exigía **email único** vía `emailExists()`, una regla que ponía solo el PHP — `mail_addr` es `varchar(50) NULL` sin constraint y la PK de `MEMB_INFO` es `memb___id`, así que la base siempre permitió repetirlo. Reemplazado por un techo configurable, `AccountRepository::MAX_CUENTAS_POR_EMAIL = 5`, con `countByEmail()` nuevo; `account/changeemail.php` usa la misma constante, si no quedaba la inconsistencia de poder registrar 5 cuentas con un email pero no poder ponerle ese email a una cuenta existente. (2) El token de **Turnstile es de un solo uso**: al reintentar en la misma página (crear otra cuenta, o reintentar después de un error de validación) el widget no se renovaba y fallaba siempre con 'Verificación de seguridad fallida', sin salida salvo recargar a mano — `register.js` ahora llama `turnstile.reset()` en todas las respuestas. De paso se sacó la redirección automática al login post-registro: mandaba afuera a quien estaba creando varias cuentas seguidas; ahora el formulario se limpia, vuelve el foco al usuario y el link a login queda abajo como siempre. No hay recuperación de contraseña por email en el sitio, así que repetir emails no rompe ningún flujo (verificado con grep sobre `mail_addr`). Requiere `git pull` en el VPS (API) **y** deploy de Cloudflare (register.js).
- 2026-09-04 — [Feat] Pantalla de apertura: **`/info/` destapada** a pedido de Franco (es lo que la gente quiere leer antes de arrancar: rates y características del server). Tres puntos, todos atados a la misma condición horaria: `/info` sumada a `esRescate` en `apertura.js` (franja con el contador en vez de pantalla completa), sumada a la allowlist de `filtrarNav()` (vuelve el link Info al navbar) y `site/server-info.php` exento del 503 en `AperturaGate.php`. Requiere `git pull` en el VPS (API) y deploy de Cloudflare (apertura.js). Verificado con captura headless: nav = Info · Descargas · Login · Registrarme (+ Admin/Salir ocultos) y franja contando.
- 2026-09-04 — [Feat] `/donate2/`: donación **por transferencia** con modal de promos, a pedido de Franco (hasta ahora todo salía por link de pago de Mercado Pago). Card nueva "Transferencia bancaria" primera en la grilla, con `transfer_popup` en `data/donate.json` (alias, promos monto→WCoins con su bonificación, nota de monto libre y los links de contacto) — mismo criterio data-driven que el resto de la página, no hay nada hardcodeado en el template. El modal deja explícitas las dos cosas que se saltean: **transferir el monto exacto** de la promo (si no, no se aplica la bonificación) y **avisarle a un administrador** con el comprobante (la acreditación es manual). Reusa el modal del QR de Binance: `donate2.js` pasó de tener ese modal cableado a mano a un `wireModal(id, selector)` genérico — antes un `return` temprano cortaba todo si la página no tenía card de Binance — más `wireCopyAlias()` con fallback a selección de texto si no hay permiso de portapapeles. El card de link de pago de MP se mantiene como segunda opción (se saca borrando su bloque del JSON, nada más). [Fix] De paso, `.transfer-alias-value span` pasó de la fuente display a monoespaciada: con Cinzel el punto de `MUPGA.MP` se perdía entre las mayúsculas y se leía "MUPGAMP" — es un alias que tienen que copiar exacto, y el mismo problema estaba en `/donate/transferencia/`. Verificado con capturas en 360/390/1100. Solo frontend: no requiere pull en el VPS.
- 2026-09-04 — [Feat] Pantalla de apertura: **`/donate2/` (WCoins) destapada** a pedido de Franco, para que puedan donar desde antes de abrir. Solo frontend, sin exención de API necesaria: la página no llama a ningún endpoint — el contenido sale de `data/donate.json` horneado en el build, y los links de pago, el alias y el modal de transferencia son estáticos. `/donate2` sumada a `esRescate` (franja con el contador) y a la allowlist de `filtrarNav()` (vuelve el link WCoin al navbar). El redirect a login de `donate2.js` manda `?redirect=/donate2/`, que `login.js` respeta por encima del destino por defecto de la ventana de apertura, así que después de loguearse vuelven a la página de donaciones.
- 2026-09-04 — [Feat] `/donate2/`: **fuera los links de pago** a pedido de Franco — se borró la card de Mercado Pago con sus 5 links `mpago.la`, así las donaciones en pesos van todas por transferencia al alias. Quedan Transferencia bancaria (con el modal de promos), Binance Pay (QR, no es link de pago) y Western Union (se coordina por contacto). El `highlight` de la card de transferencia decía "mismas bonificaciones que el link de pago", que quedaba colgado sin esa card: ahora dice "⭐ Con bonificación según el monto". Solo se editó `data/donate.json` — el template y el JS no cambiaron, así que restaurar los links es volver a poner ese bloque (está en el historial de git, commit 4e3d6d1 y anteriores). Solo frontend.
- 2026-09-04 — [Feat] Usercp: **opciones de personaje grisadas** a pedido de Franco — Unstick, Limpiar PK, Resetear Stats y Reset de personaje quedan deshabilitadas (el de Árbol ML ya lo estaba). Interruptor único en `src/config/char_actions.php` (mismo criterio de fuente-única-de-verdad que `apertura.php`): el array `CHAR_ACTIONS_BLOQUEADAS` decide a la vez el botón grisado con su leyenda ("Próximamente disponible") en `usercp/index.php` y el 403 de `enforceCharAction()` en los cinco `api/account/*.php`, insertado antes de `requireAuth()` para que nadie llegue a la DB con curl. `usercp.js` no sabe qué está cerrado: lee `data-locked="1"` del HTML, y con eso ni engancha listener ni rehabilita el botón al elegir personaje — de paso `btn-resetml` volvió a la tabla de acciones y a `actionBtns`, que estaba comentado a mano. Fail-closed a propósito, al revés que `AperturaGate`: si falta el config, el `require_once` tira 500 y la opción queda cerrada; un deploy a medias no puede reabrirla. Para rehabilitar una: sacarle la línea al array + push (frontend) y `git pull` en el VPS (API). Requiere las dos capas.
