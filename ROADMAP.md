...
- 2026-09-01 — [Design] Pergamino "GRAN APERTURA MUPGA 04/09" flotando sobre el hero del inicio, para la apertura del 04/09. Es un elemento propio (`.hero-scroll`, hermano de `.hero-slider`), independiente del slider: se suma `hero-apertura-scroll.png` (PNG con alfa) con animación CSS de respiración (float ±12px + balanceo ±0.7° en 5s, más un halo dorado/violeta que pulsa en sincro; respeta `prefers-reduced-motion`) y doble `drop-shadow` — la corta le hace contorno para que no se funda con los slides claros (img-3 nieve, img-5 cyan, img-6 gris). El slider rotativo se mantiene completo, ahora encabezado por `hero-apertura.jpg` (key art generado con Gemini, reescalado 2752x1536 → 2560x1429, JPG q85, 341 KB); `hero-slider.js` no se tocó. Lo no obvio del posicionamiento: el hero **no** ocupa el viewport completo (la sidebar de 272px se lo come hasta los 1100px, donde además desaparece) y el `.hero-content` mide 580px fijos, así que el hueco entre el texto y el borde derecho solo existe a partir de ~1850px de viewport — abajo de eso el pergamino se va chico a la esquina superior derecha, y en ≤768px a la esquina inferior derecha. Ojo con el corte de 1850px: un monitor de 1920 con Windows al 125% de escala reporta 1536px CSS, así que cae en la variante chica. Todo validado con capturas headless en 390/360/768/1024/1280/1440/1600/1920. **Pendiente estético conocido** (preexistente, no lo introduce este cambio): los 10 slides viejos traen "MUPGA Season 6" quemado en la imagen y se pisa con el `<h1>MuPGA</h1>` del hero; el pergamino queda al lado de ese título, así que en esos slides hay tres bloques de texto dorado a la vez. Si molesta, la salida más barata es bajar el pergamino a la esquina inferior derecha también en desktop.
