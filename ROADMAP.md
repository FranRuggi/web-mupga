...
- 2026-09-01 — [Design] Hero del inicio pasa temporalmente a **una sola imagen fija** para la apertura del 04/09, a pedido de Franco: se reemplazan los 13 slides (10 img + 3 video) por `hero-apertura.jpg` (key art generado con Gemini, reescalado 2752x1536 → 2560x1429, JPG q85, 341 KB) y se suma un pergamino "GRAN APERTURA MUPGA 04/09" (`hero-apertura-scroll.png`, PNG con alfa) flotando sobre el hero con animación CSS de respiración (float ±12px + balanceo ±0.7° en 5s, más un halo dorado/violeta que pulsa en sincro; respeta `prefers-reduced-motion`). El slider viejo y `$extraJs = 'hero-slider.js'` quedan **comentados en el propio `index.php`** para poder volver atrás en un minuto cuando pase la apertura; `hero-slider.js` no se tocó (ya contempla el caso de un solo slide). Lo no obvio del posicionamiento: el hero **no** ocupa el viewport completo (la sidebar de 272px se lo come hasta los 1100px, donde además desaparece) y el `.hero-content` mide 580px fijos, así que el hueco entre el texto y el caballero solo existe a partir de ~1850px de viewport — abajo de eso el pergamino se va chico a la esquina superior derecha, sobre el ala (color plano), y en ≤768px a la esquina inferior derecha. Ojo con el corte de 1850px: un monitor de 1920 con Windows al 125% de escala reporta 1536px CSS, así que cae en la variante chica. Todo validado con capturas headless en 390/360/768/1024/1280/1440/1600/1920.
