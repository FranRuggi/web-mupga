...
- [ ] Configurar en la API externa el `paymentUrl` del proveedor "Transferencia Bancaria" para que apunte a `https://mupga.com.ar/donate/transferencia/` (ver `.claude/docs/payment_integration.md`, Paso 5)
- [x] Nav (`layout.php`) y `usercp/index.php` repuntados a `/donate/` — 2026-08-05
- [ ] Limpieza final pendiente: eliminar `src/public/donate2/`, `src/public/assets/js/donate2.js` y `data/donate.json`; quitar `donate2/index.html` de `build.php` (se dejó todo en el repo por ahora como rollback rápido)
