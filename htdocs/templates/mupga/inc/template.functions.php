...
function templateCastleSiegeWidget() {
	...
	echo '<div class="panel castle-owner-widget">';
		...
		echo '<div class="panel-body">';
			echo '<div class="castle-owner-grid">';
				echo '<div class="castle-owner-col">';
					echo '<div class="castle-owner-logo">';
						echo returnGuildLogo($guildOwnerMark, 100);
					echo '</div>';
					echo '<div class="castle-owner-text">';
					...
					echo '</div>';
				echo '</div>';
				echo '<div class="castle-stage-col">';
					...
					echo $siegeData['warfare_stage_countdown'];
				echo '</div>';
			echo '</div>';
			echo '<div class="castle-owner-actions">';
...
function templateClassInfo() {
	$database = Connection::Database('MuOnline');
	$query = $database->query_fetch("SELECT "._CLMN_CHR_CLASS_." FROM "._TBL_CHR_." WHERE "._CLMN_CHR_LVL_." > 6");
	if($query) {
		return $query;
	}
	return array();
...
function templateLanguageSelector() {
	...
	// Backward compatibility if config/session still uses "br"
	if($lang === 'br') {
		$lang = 'pt';
	}
	...
	echo '<ul class="webengine-language-switcher">';
		echo '<li><a href="'.__BASE_URL__.'language/switch/to/'.strtolower($lang).'" title="'.$langList[$lang][0].'"><img src="'.getCountryFlag($langList[$lang][1]).'" /></a></li> ';
		foreach($langList as $language => $languageInfo) {
			if($language == $lang) continue;
			echo '<li><a href="'.__BASE_URL__.'language/switch/to/'.strtolower($language).'" title="'.$languageInfo[0].'"><img src="'.getCountryFlag($languageInfo[1]).'" /></a></li> ';
		}
	echo '</ul>';
}